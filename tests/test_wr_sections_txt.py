...
def test_wr_sections_all(tmp_path):
    ...
    f_sec_wr = tmp_path / "tmp_test_sections_out.txt"
    f_sec_py = tmp_path / "tmp_test_sections.py"
    ...
    objsecwr.wr_txt_section_hdrgos(str(f_sec_wr))
    objsecpy = WrSectionsPy(grprobj)
    objsecpy.wr_py_sections(str(f_sec_py), sec_rd, doc=godag.version)
    # Read text and Python sections files
    _read_sections(str(f_sec_wr))
    _read_sections(str(f_sec_py))
...
def test_wr_sections_txt(tmp_path):
    ...
    _wr_sections_txt(tmp_path / "a_sec0_hdr1.txt", usrgos, sections=None, grprdflt=grprdflt)

    # ------------------------------------------------------------------
    # Print usrgos in txt using sections containing hdrgos
    # ------------------------------------------------------------------
    sec1 = _read_sections("./data/gjoneska_pfenning/sections_in.txt")
    _wr_sections_txt(tmp_path / "a_sec1_hdr1.txt", usrgos, sec1, grprdflt=grprdflt)

    # ------------------------------------------------------------------
    sec2a = _read_sections("goatools/test_data/sections/gjoneska_pfenning.py")
    _wr_sections_txt(tmp_path / "b_sec1_hdr1.txt", usrgos, sec2a, grprdflt=grprdflt)

    sec2b = _read_sections("goatools.test_data.sections.gjoneska_pfenning")
    _wr_sections_txt(tmp_path / "c_sec1_hdr1.txt", usrgos, sec2b, grprdflt=grprdflt)
...
def _wr_sections_txt(fout_txt, usrgos, sections, grprdflt):
    ...
    try:
        full_txt = os.fspath(fout_txt)
        if not os.path.isabs(full_txt):
            full_txt = os.path.join(REPO, full_txt)
        hdrobj = HdrgosSections(
            grprdflt.gosubdag, grprdflt.hdrgos_dflt, sections=sections
        )
        grprobj = Grouper(os.path.basename(full_txt), usrgos, hdrobj, grprdflt.gosubdag, go2nt=None)
...
if __name__ == "__main__":
    with tempfile.TemporaryDirectory() as tmpdir:
        outdir = Path(tmpdir)
        test_wr_sections_all(outdir)
        test_wr_sections_txt(outdir)
