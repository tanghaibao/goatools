...
from goatools.test_data.sorter import SECTIONS, USER_GOS
...
def test_sort(tmp_path):
    ...
    _wr_xlsx(tmp_path, "t_a1_ps1_ph1", grprobj)
    #   * Omit headers (retain "a_ph1" sort)
    _wr_xlsx(tmp_path, "t_a2_ps1_ph0", grprobj, hdrgo_prt=False)
    # 2) Print section as an additional column instead of a section row.
    #   * Use and print headers.
    _wr_xlsx(tmp_path, "t_a3_ps0_ph1", grprobj, section_prt=False)
    #   * Use (but don't print) headers.
    _wr_xlsx(tmp_path, "t_a4_ps0_ph0", grprobj, section_prt=False, hdrgo_prt=False)

    # ------------------------------------------------------------
    # TEST: use_sections=False
    #       sections are ignored, but hdrgos defined in sections are used
    # ------------------------------------------------------------
    _wr_xlsx(tmp_path, "t_b1_us0_ph1", grprobj, use_sections=False)
    _wr_xlsx(tmp_path, "t_b2_us0_ph0", grprobj, use_sections=False, hdrgo_prt=False)
    ...
    _wr_xlsx(tmp_path, "t_c1_ps1_ph0_hsortT", grprobj, section_sortby=True)
    _wr_xlsx(tmp_path, "t_c2_ps1_ph0_hsortT_top3", grprobj, section_sortby=True, top_n=3)
    # Not commonly used: Uses order from a1_ps1_ph1
    _wr_xlsx(tmp_path, "t_c3_ps1_ph0_hsortX_top5", grprobj, top_n=5)
    # Most commonly used; User provides the sort. Users often like to sort by pval, if exists:
    _wr_xlsx(tmp_path, "t_c4_ps1_ph0_usort", grprobj, section_sortby=sortby)
    _wr_xlsx(tmp_path, "t_c5_ps1_ph0_usort_top3", grprobj, section_sortby=sortby, top_n=3)
    _wr_xlsx(
        tmp_path,
        "t_c6_ps0_ph0_usort_top3",
        grprobj,
        section_sortby=sortby,
        top_n=3,
        section_prt=False,
    )


def _wr_xlsx(outdir, name, grprobj, **kws):
    ...
    fout_xlsx = outdir / "{NAME}.xlsx".format(NAME=name)
    ...
    objwr.wr_xlsx_nts(str(fout_xlsx), desc2nts, **kws)


def _get_grprobj():
    """Get object for grouping GO IDs."""
    fin_obo = os.path.join(REPO, "go-basic.obo")
    godag = get_godag(fin_obo, prt=None, optional_attrs=["relationship"])
