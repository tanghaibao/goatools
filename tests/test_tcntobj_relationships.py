...
def test_tcntobj_relationships(prt=sys.stdout):
    ...
    ns2tcntobj_r1 = {
        ns: TermCounts(go2obj_r1, annoobj.get_id2gos(ns), RELS) for ns in NSS
    }
...
def _chk_pass_fail(ns2tcntobj_r0, ns2tcntobj_r1):
    ...
    print(
        "\nCOMPARE GO Counts wo/relationships and with: {Rs}".format(
            Rs=" ".join(sorted(RELS))
        )
    )
    for nspc in NSS:
        ...
        for goid, cnt_r0 in ns2tcntobj_r0[nspc].gocnts.most_common():
            ...
            if cnt_r0 != cnt_r1:
                cnt += 1
        print(
            "{NS:3} {N:5,} more GO ID counts using relationships".format(NS=nspc, N=cnt)
        )
