...
def _chk_key(a2bs, pattern):
    ...
    for key in a2bs.keys():
        if pattern in key:
            return
        raise RuntimeError(
            "PATTERN({P}) NOT FOUND IN KEY({K})".format(P=pattern, K=key)
        )
