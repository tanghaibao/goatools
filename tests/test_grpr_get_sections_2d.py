...
def test_fnc():
    """Test function, get_sections_2d, in the Grouper class."""
    usrgo2nt = {
        getattr(nt, "GO"): nt for nt in goea_results if getattr(nt, "p_fdr_bh") < 0.05
    }
...
def chk_results(sections_act, grprobj):
    ...
    PRT.write(
        "{N} of {M} Sections Header GOs for {U} user GOs, {H} headers\n".format(
            N=len(hdrgos_sec_act),
            M=num_gos_orig,
            U=len(grprobj.usrgos),
            H=len(hdrgos_act),
        )
    )
