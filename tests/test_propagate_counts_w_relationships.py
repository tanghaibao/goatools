...
def test_pc_w_rels(prt=sys.stdout):
    ...
    godag_r0 = get_godag(file_obo, prt)
    godag_r1 = get_godag(file_obo, prt, optional_attrs=["relationship"])

    # pylint: disable=line-too-long
    # Check that relationships are used in propgate counts
    results_r0 = _get_results(
        godag_r1, propagate_counts=True, relationships=False, prt=prt
    )
    results_r1 = _get_results(
        godag_r1, propagate_counts=True, relationships=True, prt=prt
    )
    results_rr = _get_results(
        godag_r1,
        propagate_counts=True,
        relationships={"regulates", "negatively_regulates", "positively_regulates"},
        prt=prt,
    )
    results_rp = _get_results(
        godag_r1, propagate_counts=True, relationships={"part_of"}, prt=prt
    )
