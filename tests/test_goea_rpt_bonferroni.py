...
def test_bonferroni():
    ...
    prtfmt = " ".join(
        [
            "{NS} {study_count:3} {p_uncorrected:5.3e}",
...
def run_bonferroni():
    ...
    popul_ids = [
        line.rstrip()
        for line in open(os.path.join(REPO, "data/population"), encoding="utf-8")
    ]
    study_ids = [
        line.rstrip()
        for line in open(os.path.join(REPO, "data/study"), encoding="utf-8")
    ]
    # 2. Run enrichment analysis
    goea = GOEnrichmentStudy(
        popul_ids, assoc, godag, alpha=0.05, methods=["bonferroni"]
    )
