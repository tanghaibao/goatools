...
def test_find_enrichment():
    ...
    pop = set(_.strip() for _ in open(fin_genes, encoding="utf-8") if _.strip())
    stu_orig = pop
    num_pop = len(pop)
    objtest = ArgsDict()
    get_godag(objtest.namespace["obo"])
    for min_overlap in [0.25, 0.50, 0.75]:
        ...
        print(
            "{N:3} of {M} ({OL}%) in study in pop".format(
