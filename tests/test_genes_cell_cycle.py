...
def get_genes_cell_cycle(taxid=9606, log=sys.stdout):
    ...
    with open(fout_allgos, "w", encoding="utf-8") as prt:
        ...
        if log is not None:
            ...
            log.write(
                "    FOUND {N:>5} GOs:   {F}\n".format(N=len(gos_all), F=fout_allgos)
            )
...
def prt_genes(fout_genes, geneids, taxid, log):
    ...
    if os.path.isfile(fin_symbols):
        ...
        with open(fout_genes, "w", encoding="utf-8") as prt:
            prt_txt(prt, nts, fmtstr)
            if log is not None:
                log.write(
                    "    WROTE {N:>5} genes: {FOUT}\n".format(
                        FOUT=fout_genes, N=len(nts)
                    )
                )
    # Just print geneids
    else:
