...
def test_gosubdag_relationships(wr_new_obo_subset=False):
    ...
    godag_r0 = get_godag(fin_obo)
    godag_r1 = get_godag(fin_obo, optional_attrs=["relationship"])
    ...
    assert (
        "GO:0016441" not in gosubdag_rp.go2obj
    ), "**FATAL: REGULATION TERM GoSubDag(part_of) go2obj"
    assert (
        "GO:0016441" not in rp_par
    ), "**FATAL: REGULATION TERM GoSubDag(part_of) go2parents"
    ...
    gosubdag_rp = GoSubDag(
        set([goid_chosen]), godag_r1, relationships={"positively_regulates"}
    )
    assert gosubdag_rp.relationships == set(["positively_regulates"])
    rp_par = gosubdag_rp.rcntobj.go2ancestors[goid_chosen]

    # RELATIONSHIPS: negatively_regulates
    gosubdag_rn = GoSubDag(
        set([goid_chosen]), godag_r1, relationships={"negatively_regulates"}
    )
...
def _run_baseline_r0(gosubdag_r0, gosubdag_r1):
    ...
    print(
        "{N} r1 GO terms in GoSubDag have more ancestors than r0".format(
    ...
    with open(fout_gos, "w", encoding="utf-8") as prt:
        prt.write(
...
def _prt_goterms(goids, go2nt, prt):
    ...
    fmt = (
        "#ffd1df {GO} # {NS} {dcnt:5} {childcnt:3} "
...
def _wr_sub_obo(fout_obo, goid_chosen, godag_r1, fin_obo):
    ...
    with open(fout_obo, "w", encoding="utf-8") as prt:
    ...
    system(
        pat_r1.format(
            REPO=REPO, PNG=fout_obo.replace(".obo", "_r1.png"), GO=goid_chosen
        )
    )
    system(
        pat_r0.format(
            REPO=REPO, PNG=fout_obo.replace(".obo", "_r0.png"), GO=goid_chosen
        )
    )
...
def _get_leafs_w_relsinhier(rels_usr, gosubdag_r1):
    ...
    for goid in goids_leaf:
        ...
        rels = set(
            k for p in go_parents for k in gosubdag_r1.go2obj[p].relationship.keys()
        )
        if rels == rels_usr:
