...
def _chk_pvals(fisher2pvals, prt):
    ...
    for fish1, fish2 in combinations(fisher2pvals.keys(), 2):
        ctr = cx.Counter()
        pvals1 = cx.OrderedDict(
            sorted([(r.GO, r.p_uncorrected) for r in fisher2pvals[fish1]])
        )
        pvals2 = cx.OrderedDict(
            sorted([(r.GO, r.p_uncorrected) for r in fisher2pvals[fish2]])
        )
...
def _get_pvals(pvalfnc_names, prt=sys.stdout):
    ...
    obo_dag = get_godag(file_obo, prt)
