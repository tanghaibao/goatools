...
def test_dflthdrs(prt=sys.stdout, do_plt=False):
    ...
    hdrobj0 = HdrgosSections(
        grprdflt.gosubdag, grprdflt.hdrgos_dflt, sections=None, hdrgos=None
    )
    ...
    name = "usrhdrs4"
    hdrobj1 = HdrgosSections(
        grprdflt.gosubdag, grprdflt.hdrgos_dflt, sections=None, hdrgos=hdrgos
    )
...
def run(grprobj, hdrobj, exp_hdrs, hdrgo_prt=True):
    ...
    sys.stdout.write(
        "\n{NAME} PRINT GOs hdrgo_prt({H}):\n".format(H=hdrgo_prt, NAME=grprobj.grpname)
    )
    sortobj.prt_gos()
    ...
    assert (
        nts_go.intersection(usrgos) == usrgos
...
def chk_hdrs(grprobj, hdrobj, prt=sys.stdout):
    ...
    prt.write(
        "{N} hdrgos ({U} are also user GO IDs) used out of {M} available\n".format(
...
def get_data0():
    ...
    )  # BP 0    22 L03 D05 DKO learning or memory


def _get_gosubdag():
    """Get GO DAG."""
    fin = os.path.join(REPO, "go-basic.obo")
    godag = get_godag(fin, prt=sys.stdout, optional_attrs=["relationship"])
