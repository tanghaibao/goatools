...
class _Run:
    ...
    def __init__(self):
        download_go_basic_obo(self.obo, sys.stdout)
        ...
        prt_hms(
            tic,
            "GoSubDag r0 {N:4} GOs {S:3} srcs".format(
                N=len(self.gosubdag_r0.go2obj), S=len(self.gosubdag_r0.go_sources)
            ),
        )
        # GoSubDag with relationships
        self.gosubdag_r1 = GoSubDag(
            self.goids, self.godag_r1, prt=None, relationships=True
        )
        prt_hms(
            tic,
            "GoSubDag r1 {N:4} GOs {S:3} srcs".format(
