...
def _combine_nt_vals(nts, flds, dflt_null):
    """Take a list of go2nts, return combined values for a single namedtuple."""
    vals = []
    # 2a. Collect values for each namedtuple field
    for fld in flds:
        fld_seen = False
        for id2nt in id2nts:
            nt_curr = id2nt.get(item_id)
            if nt_curr is not None: 
    ...
    for fld in flds:
        ...
        for ntitem in lst0_lstn:
            if hasattr(ntitem, fld):
                vals.append(getattr(ntitem, fld))
                fld_seen = True
                break
        if fld_seen is False:
            vals.append(dflt_null) # Default val if GO id or nt val is not present
