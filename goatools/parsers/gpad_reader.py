...
class GpadReader(object):
    ...
    gpadhdr = [ #              Col Req?     Cardinality    Example
        ...
        # DB_Reference: set([''DOI:10.1002/sita.200600112', 'GO_REF:0000037', 'Reactome:R-HSA-6814682'])
    ...
    def _get_ntgpad(self, ntgpadobj, flds, ver):
    ...
    def _get_extensions(self, extline):
        ...
        for ext_lst in extline.split('|'):
            ...
            for ext in ext_lst.split(','):
                idx = ext.find('(')
                assert idx != -1 and ext[-1] == ')', 'BAD Extension({E})'.format(E=ext)
    ...
    def read_gpad(self, fin_gpad, hdr_only=False):
        ...
        for line in ifstrm:
            ...
            if ntgpadobj is not None:
                ...
                ntgpad = self._get_ntgpad(ntgpadobj, flds, ver)
